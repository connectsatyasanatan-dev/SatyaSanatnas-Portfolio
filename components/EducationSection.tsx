...
const EducationSection = () => {
    ...
    if (loading) return <EducationSectionSkeleton />
    if (!loading && education.length === 0) return null

    const active = education[activeIndex]
    const totalCourses = education.reduce((a, e) => a + (e.relevant_courses?.length || 0), 0)
    const totalAchievements = education.reduce((a, e) => a + (e.achievements?.length || 0), 0)
